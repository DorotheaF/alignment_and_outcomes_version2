...
def InitialCleanup(dataframe,
    ...
    # only allow strings, spaces, and newlines to pass (addind digits)
    ...
    def applyRegExpression(textFiller):
        textClean = re.sub('^(?!mom|am|ham)[u*|h*|m*|o*|a*]+[m*|h*|u*|a*]+\s', ' ',
                           textFiller)  # at the start of a string
        ...
        if len(dataframe) > 0:
            while k < len(dataframe) - 1:
                if dataframe['participant'].iloc[k] != dataframe['participant'].iloc[k + 1]:
                    ...
                    DfMerge.append([dataframe['participant'].iloc[k],
...
def Lemmatize(tokenlist):
    ...
    for item in defaultPos:
        words_lemma.append(lemmatizer.lemmatize(item[0], pos_to_wn(
            item[1])))  # need to convert POS tags to a format (NOUN, VERB, ADV, ADJ) that wordnet uses to lemmatize
    return words_lemma
...
def ApplyPOSTagging(df,
    ...
    if add_stanford_tags:
        if stanford_pos_path is None or stanford_language_path is None:
            raise ValueError(
                'Error! Specify path to Stanford POS tagger and language model using the `stanford_pos_path` and `stanford_language_path` arguments')
        else:
